...
# CSV 대신 parquet로 변경된 경로
COMMERCIAL_DATA_PATH = "hdfs://master1:9000/data/commercial_data.parquet"
...
def load_commercial_data(spark):
    ...
    if cached_commercial_data is None:
        print("Parquet 상권 데이터 로드 및 캐싱 중...")
        cached_commercial_data = spark.read.parquet(COMMERCIAL_DATA_PATH).cache()
        print("Parquet 상권 데이터 캐싱 완료")
